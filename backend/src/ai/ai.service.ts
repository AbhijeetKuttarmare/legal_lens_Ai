...
export const TEMPLATE_TYPES: Record<string, string> = {
  ...
  FREELANCE_CONTRACT: 'Freelance / Independent Contractor Agreement',
};

function templateSystemPrompt(templateLabel: string, language?: string): string {
  const name = languageName(language);
...
- Use standard, fair, commonly-used clauses for this document type under Indian law where relevant.
  ...
  async generateTemplate(
    ...
    try {
      ...
      const response = await this.client.messages.create({
        ...
        max_tokens: 4096,
        thinking: { type: 'disabled' },
        system: templateSystemPrompt(templateLabel, language),
